...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public SwerveSubsystem(File directory)
  {
    ...
    try
    {
      swerveDrive = new SwerveParser(directory).createSwerveDrive(Constants.DrivebaseConstants.MAX_SPEED);
  ...
  {
    swerveDrive = new SwerveDrive(driveCfg, controllerCfg, Constants.DrivebaseConstants.MAX_SPEED);
  }
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public void updateVisionOdometry()
  {
    ...
    {
      LimelightHelpers.SetRobotOrientation(Constants.Limelight.FRONT, swerveDrive.swerveDrivePoseEstimator.getEstimatedPosition().getRotation().getDegrees(), 0, 0, 0, 0, 0);
      LimelightHelpers.PoseEstimate mt2 = LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(Constants.Limelight.FRONT);
      if(Math.abs(swerveDrive.getGyro().getRate()) > 720) // if our angular velocity is greater than 720 degrees per second, ignore vision updates
      {
        doRejectUpdate = true;
      }
      if(mt2.tagCount == 0)
